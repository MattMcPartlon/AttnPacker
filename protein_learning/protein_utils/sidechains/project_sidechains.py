...
def project_onto_rotamers(
    ...
    for eval_iter in range(optim_repeats):
        ...
        def loss_closure():
            ...
            loss_val = rmsd_loss_val + (steric_loss_val * steric_clash_weight) + (dihedral_dev*torsion_deviation_loss_wt)
            loss_val.backward()
            return loss_val
...
def iterative_project_onto_rotamers(
    ...
    **kwargs
    ...
    for project_iter in range(iterative_refine_max_iters):
        ...
        prev_atom_coords[~ignore_mask] = alpha*prev_atom_coords[~ignore_mask].detach() \
