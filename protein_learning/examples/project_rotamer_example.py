...
def project_pdb(
    ...
    if pdb_path_out is None:
        pdb_path_out = os.path.join(
            os.path.dirname(pdb_path_in),f"post-processed-{os.path.basename(pdb_path_in)}"
        )
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Project Protein Sidechains onto Continuous Rotamer and Minimize Steric Clashes",  # noqa
                        epilog='',
                        formatter_class=ArgumentDefaultsHelpFormatter)

    parser.add_argument("pdb_path_in",help="path to input pdb")
    parser.add_argument("--pdb_path_out",help="path to save projected pdb to (dafaults to post-processed-<input pdb name>.pdb")
    parser.add_argument("--steric_wt",help="weight to use for steric clash loss",default=0.2,type=float)
    parser.add_argument("--steric_tol_allowance",help="subtract this number from all atom vdW radii",default=0.05,type=float)
    parser.add_argument("--steric_tol_frac",help="set vdW radii to steric_tol_frac*vdW(atom_type)",default=0.925,type=float)
    parser.add_argument("--steric_hbond_allowance",help="subtract this number from the sum of vdW radii for hydrogen bond donor/acceptor pairs",default=0.6,type=float)
    parser.add_argument("--max_optim_iters",help="maximum number of iterations to run optimization procedure for",type=int,default=250)
    parser.add_argument("--torsion_loss_wt",help="penalize average deviaiton from initial dihedral angles with this weight",type=float, default=0)
