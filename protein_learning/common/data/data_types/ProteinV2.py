...
class Protein:
    ...
    def __init__(
        ...
        self.chain_ids = default(
            chain_ids,
            torch.zeros(len(seq), device=atom_coords.device)
        )
        assert len(self.chain_ids) == len(seq)
        self.crops, self.input_partition = None, None
        ...
        if exists(cdrs):
            for s, e in cdrs["heavy"]:
                assert s > 0 and e > 0, f"{cdrs}"
    ...
    def make_antibody(
        ...
        heavy, light, ag = map(
            ...
            ) if exists(x[0]) else None, zip((_heavy, _light, _ag), (seqs))
        )
