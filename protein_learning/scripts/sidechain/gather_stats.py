...
def calc_native_plddt(tgt_stats):
    ...
    mask = c_mask & mask
    pred, actual = map(
        lambda x: x[mask].unsqueeze(0), (pred, actual)
    )
    ...
    for target in targets:
        try:
            ...
            if not os.path.exists(decoy_pdb):
                ...
                continue
            method_stats[target] = assess_sidechains(target_pdb_path=native_pdb,decoy_pdb_path=decoy_pdb,steric_tol_allowance=0.01)
...
def get_stats_for_tgt(assess_stats):
    ...
    return dict(
        ...
        clash_info = {tol:{k: assess_stats['clash-info'][tol]["steric"][k] for k in steric_ks} for tol in assess_stats['clash-info']},
