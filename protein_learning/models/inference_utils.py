...
import attnpacker.models.model_abc.train as sc
from attnpacker.models.utils.model_io import (
    get_args_n_groups,
    load_args_for_eval,
)
from attnpacker.models.fbb_design.train import Train as SCPTrain, _augment
...
def format_prediction(model, model_in, model_out, device="cpu", detach=False):
    ...
    out = dict(
        ...
        encoder_res_output=model_out.encoder_residue,
        design_mask=seq_mask,
        model_out=model_out.detach() if detach else model_out,
        model_in=model_in,
...
class Inference(L.LightningModule):
    def __init__(
        self,
    ...
    def load_example(
        ...
        atom_masks: Tensor = None,
        seq: str = None,
        name: str = "",
        device="cpu",
    ):
        if exists(coords):
            assert coords.ndim == 3, f"does not work batched :( {coords.shape}"
            # offset residue ids for each chain
            res_ids = []
            for i in range(int(torch.max(chain_ids).item()) + 1):
                res_ids.append((100 * i) + torch.arange(torch.sum((chain_ids == i))))
    ...
    def infer(
        ...
        **example_kwargs,
        ...
        with torch.set_grad_enabled(enable_grad and model.training):
            ...
            if len(model_input.decoy) <= chunk_size:
                model_output = model(model_input.to(self.device), use_cycles=1)
            else:
